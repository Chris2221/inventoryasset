...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submitRequest'])) {
    ...
    $query = "UPDATE OutboundAssets 
              SET Descriptions = ?, DateAcquired = ?, ExpectedReturnDate = ?, DepartureDate = ?, ExpectedReceiver = ?, FK_Users = ?, Approvals = ?, Status = ?, CreatedBy = ? $imageSql 
              WHERE PK_OutboundAssets = ?";

    if ($imageSql) {
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssissssi", $description, $dateAcquired, $expectedReturn, $departureDate, $receiveBy, $fkUser, $approvals, $status, $createdBy, $imagePath, $outboundId);
    } else {
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssssisssi", $description, $dateAcquired, $expectedReturn, $departureDate, $receiveBy, $fkUser, $approvals, $status, $createdBy, $outboundId);
    }

    if ($stmt->execute()) {
        // Delete previous entries in OutboundAssetsList
        $conn->query("DELETE FROM OutboundAssetsList WHERE FK_OutboundAssets = $outboundId");
        ...
        header("Location: outbound-assets.php?status=requestupdated");
        exit();
    } else {
...
$expectedReturn = $outbound['ExpectedReturnDate'] ?? '';
...
<body>
    ...
    <div class="container">
        ...
        <div class="d-flex justify-content-start mb-3">
            <?php if (!$inbound) { ?>
                <a href="outbound-assets.php?Status=<?php echo $status ?>" class="btn btn-dark"><i class="bi bi-arrow-left-circle me-2"></i>Back</a>
            <?php } else { ?>
                <a href="inbound-assets.php?Status=<?php echo $status ?>" class="btn btn-dark"><i class="bi bi-arrow-left-circle me-2"></i>Back</a>
            <?php } ?>
                    ...
                    <div class="row g-4">
                        <div class="col-lg-6">
                            ...
                            <div class="mb-3 mt-3">
                                ...
                                <input type="date" name="expectedReturn" id="expectedReturn" class="form-control" value="<?php echo $expectedReturn; ?>" required
                                    <?php
                                    if ($isApproved) { ?>
                                    disabled
                                    <?php
