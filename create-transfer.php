...
function getUpdatedApproverJsonWithStatus($allGAM, $conn)
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submitRequest'])) {
    ...
    $updatedApprovers = getUpdatedApproverJsonWithStatus($allGAM, $conn);
    ...
    // Insert into DB
    $stmt = $conn->prepare("INSERT INTO TransferAssets (Descriptions, Image, DateAcquired, DepartureDate, ExpectedReceiver, FK_Users, Approvals, Status, CreatedBy, Approvers) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssissis", $description, $imagePath, $dateAcquired, $departureDate, $receiveBy, $fkUser, $approvals, $status, $createdB, $updatedApprovers);
    ...
    if ($stmt->execute()) {
        ...
        TransferApproval($lastId, $conn);


        $logDetails = "Created Transfer Request ID: $lastId";

        $currentUser = $_SESSION['user_id'];
        $actionUser = "Transfer Request";

        logActivity($conn, $currentUser, $actionUser, $logDetails);

        header("Location: transfer-assets.php?status=requestadded");
        exit();
    } else {
...
<body>
    ...
    <div class="container">
        <h2 class="mb-4">Create Transfer Request</h2>

        <?php include 'modal.php'; ?>

        <div class="d-flex justify-content-start mb-3">
            <a href="transfer-assets.php" class="btn btn-dark add-btn">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" id="receiveBy" name="receiveBy" placeholder="Enter recipient name">
                                </div>
                            </div>
                        </div>
    ...
    <div id="loadingModal" class="loading-modal">
        <div class="loading-modal-content">
            ...
            <h5 class="text-xl font-semibold text-gray-800 mt-8">Requesting Asset Transfer</h5>
...
<script>
    ...
    <script>
        ...
        document.getElementById('dateRequested').setAttribute('max', todays);
    </script>
