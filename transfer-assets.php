...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['Approve'])) {
    ...
    $stmtCheck = $conn->prepare("SELECT Status, Approvers FROM TransferAssets WHERE PK_TransferAssets = ?");
    ...
    if ($status !== 'Pending') {
        header("Location: transfer-assets.php");
        exit;
    }
    ...
    if ($updated) {
        ...
        $stmtUpdate = $conn->prepare("UPDATE TransferAssets SET Approvers = ? WHERE PK_TransferAssets = ?");
        $stmtUpdate->bind_param("si", $updatedJson, $assetId);
        $stmtUpdate->execute();
        $stmtUpdate->close();

        TransferApproval($assetId, $conn);
    }


    $currentUser = $_SESSION['user_id'];
    $actionUser = "Approved Transfer";
    $logDetails = "Approved Transfer ID: REQ-$assetId";
    logActivity($conn, $currentUser, $actionUser, $logDetails);

    if ($allNowApproved) {

        $stmt = $conn->prepare("UPDATE TransferAssets SET Status = 'Approved' WHERE PK_TransferAssets = ?");
        $stmt->bind_param("i", $assetId);

        if ($stmt->execute()) {
            header("Location: transfer-assets.php?status=outboundapproved");
            exit;
        } else {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['Disapprove'])) {
    ...
    // Check if the current status is "Pending"
    $stmtCheck = $conn->prepare("SELECT Status FROM TransferAssets WHERE PK_TransferAssets = ?");
    $stmtCheck->bind_param("i", $assetId);
    $stmtCheck->execute();
    ...
    $currentUser = $_SESSION['user_id'];
    $actionUser = "Disapproved Transfer";
    $logDetails = "Disapproved Transfer ID: REQ-$assetId";
    logActivity($conn, $currentUser, $actionUser, $logDetails);

    $stmt = $conn->prepare("UPDATE TransferAssets SET Status = 'Rejected', ReasonForRejection = ? WHERE PK_TransferAssets = ?");
    $stmt->bind_param("si", $rejectionReason, $assetId);

    if ($stmt->execute()) {


        header("Location: transfer-assets.php?status=outboundrejected");
        exit;
    } else {
        echo "Error approving asset: " . $stmt->error;
    }
...
<body>
    ...
    <div class="container">
        <h2 class="mb-4">Transfer Assets</h2>

        <?php include 'modal.php'; ?>

        <div class="d-flex justify-content-end mb-3">
            <a href="create-transfer.php" class="btn btn-dark add-btn">
                        ...
                        <?php while ($row = $resultOutbounds->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="edit-transfer.php?id=<?= $row['PK_TransferAssets'] ?>&status=<?= hash('sha256', $row['Status']) ?>&type=<?= hash('sha256', 'Outbound') ?>" class="btn btn-sm btn-primary" title="View Request">
                                    ...
                                    <?php if (htmlspecialchars($row['Status']) == "Approved") { ?>
                                        <button class="btn btn-sm btn-success" onclick="generateGatePass(<?= $row['PK_TransferAssets'] ?>)" title="Generate Gate Pass">
                                    ...
                                    <?php if ($_SESSION['role'] == 'Supervisor' || $_SESSION['role'] == 'Manager' || $_SESSION['role'] == 'Admin') { ?>
                                        ...
                                        <?php if (htmlspecialchars($row['Status']) != "Approved" && htmlspecialchars($row['Status']) != "Rejected" && htmlspecialchars($row['Status']) != "Returned") { ?>
                                            <button class="btn btn-sm btn-outline-success" onclick="showApproveModal(<?= $row['PK_TransferAssets'] ?>)"
                                    ...
                                    <?php if ($_SESSION['role'] == 'Supervisor' || $_SESSION['role'] == 'Manager' || $_SESSION['role'] == 'Admin') { ?>
                                        ...
                                        <?php if (htmlspecialchars($row['Status']) != "Approved" && htmlspecialchars($row['Status']) != "Rejected" && htmlspecialchars($row['Status']) != "Returned") { ?>
                                            <button class="btn btn-sm btn-outline-danger" onclick="showDisapproveModal(<?= $row['PK_TransferAssets'] ?>)"
    ...
    <script>
        async function generateGatePass(id) {
            ...
            const response = await fetch(`generate-transfer.php?id=${id}`);
